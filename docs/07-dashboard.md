...
**Stats to load:**
- Builds: Count Query on `builds` table with user_id filter
- Signals: Count Query on `signals` table with user_id filter
- Privates: Count Query on `privates` table with user_id filter
- Privacys: Count Query on `privacys` table with user_id filter
- Keys: Count Query on `keys` table with user_id filter
- Ends: Count Query on `ends` table with user_id filter
- Messagings: Count Query on `messagings` table with user_id filter
- Voices: Count Query on `voices` table with user_id filter
...
**List Items:**
- Links to Detail Pages (/builds/{id})
...
**Actions to define:**
- "New Build" → /builds/new
- "New Signal" → /signals/new
- "New Private" → /privates/new
- "New Privacy" → /privacys/new
- "New Key" → /keys/new
- "New End" → /ends/new
- "New Messaging" → /messagings/new
- "New Voice" → /voices/new
